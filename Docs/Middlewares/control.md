...
Kd=0`、700 PWM 输出限幅；其中 1456 来自 28:1 减速比、13 线编码器的四倍频正交解码。确认
本车方向、轮径、倍率和 PID 前，
...
状态为菜单、倒计时、运行、顶点暂停、完成和故障。运行段在目标距离 80% 后启用灰度
辅助顶点判断；直线/对角要求至少两路黑线，弧线要求全白，并连续确认两个控制周期；达到
目标距离必然结束该段。所有物理量在 `RaceConfig` 统一管理，默认参数仅用于构建和初始
调试，必须实车标定。
