...
namespace {
static_assert(ENCODER_LEFT_DIRECTION == 1 || ENCODER_LEFT_DIRECTION == -1,
              "ENCODER_LEFT_DIRECTION must be 1 or -1");
static_assert(ENCODER_RIGHT_DIRECTION == 1 || ENCODER_RIGHT_DIRECTION == -1,
              "ENCODER_RIGHT_DIRECTION must be 1 or -1");
...
void sample() {
  ...
  const auto right = static_cast<std::uint8_t>(
      ...
      (DL_GPIO_readPins(GPIO_ENCODER_PORT, GPIO_ENCODER_ENCODER_RIGHT_B_PIN)
           ? 1U
           : 0U));
  update(leftTicks, leftState, left, ENCODER_LEFT_DIRECTION);
  update(rightTicks, rightState, right, ENCODER_RIGHT_DIRECTION);
}
} // namespace
namespace bsp {
